...
- **마스킹 검수 테이블**: 대시보드에 원본↔마스킹 값을 나란히 비교하는 검수용 테이블이
  있습니다. 원본(Before) 열은 기본적으로 숨겨져 있고, "🔒 원본 표시" 버튼을 눌러야만
  보이도록 해 실제 운영에서 권한이 있는 검수자만 열람하는 흐름을 흉내냈습니다. 이 테이블을
  만드는 `build_masking_comparisons()`만 예외적으로 `raw_complaints.json`을 읽으며,
  군집/집계(분석) 로직과는 분리되어 있습니다.
